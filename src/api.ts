...
function getApiBaseUrl(network: Network): string {
  return network === "testnet4" 
    ...
    : "https://atbitcoin.com/api";
}

export async function fetchProposedHandles(network: Network, query: string): Promise<string[]> {
  try {
...
export async function fetchHandlesStatuses(
  ...
  try {
    const response = await fetch(
      `${getApiBaseUrl(network)}/spaces/status`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export async function fetchHandleStatus(network: Network, handle: string): Promise<HandleStatus> {
